 ...
 */
export async function NvidiaAPI(messages: ChatCompletionMessageParam[]) {
  const openai = createNvidiaAIApi();
  const configManager = ConfigurationManager.getInstance();
  const model = configManager.getConfig<string>(ConfigKeys.NVIDIA_MODEL);
