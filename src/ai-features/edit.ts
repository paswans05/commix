...
export async function aiEdit(range?: vscode.Range) {
  ...
  const quickPickItems = [
    ...
    {
      label: '$(tools) Refactor',
      description: 'Refactor the code for better structure'
    },
        ...
        const messages: ChatCompletionMessageParam[] = [
          {
            role: 'system',
            content:
              'You are an expert coding assistant. You will receive code and an instruction. You must output ONLY the modified code that satisfies the instruction. Do not wrap it in markdown code blocks unless requested. Do not add explanations.'
          },
          {
            role: 'user',
            content: `Code:\n${selectedText}\n\nInstruction:\n${instruction}`
